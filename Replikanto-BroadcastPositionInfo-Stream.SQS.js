...
const queueUrl = "https://sqs.us-east-1.amazonaws.com/397558275354/ToTelegram.fifo";
...
async function SQSPublish(subject, msg, telegram_chat_id = undefined) {
    await new SQS().sendMessage({
        MessageAttributes: {
            ...
            MessageDeduplicationId: (new Date()).getTime().toString(36) + Math.random().toString(36).slice(2),
            MessageGroupId: "" + Math.abs(telegram_chat_id),
            QueueUrl: queueUrl
    }).promise();
}
...
async function SNSPublish2(subject, msg, telegram_chat_id = undefined) {
    await new SNS().publish({
        Subject: subject,
        Message: JSON.stringify({
            msg,
        ...
        TopicArn: 'arn:aws:sns:us-east-1:397558275354:ReplikantoToTelegram.fifo',
        MessageDeduplicationId: (new Date()).getTime().toString(36) + Math.random().toString(36).slice(2),
        MessageGroupId: "" + Math.abs(telegram_chat_id),
    }).promise();
}
...
exports.handler = async (event, context) => {
    //console.log(event);
    //console.log(context);
    //await SQSPublish("Sub", "msg", -1001715515414);
    await SNSPublish2("Sub", "msg", -1001715515414);
    const response = {
        statusCode: 200,
        body: JSON.stringify({}),
    };
